...
export async function getDataMovieWithGenre(genreId) {
    const res = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${process.env.NEXT_API_KEY}&with_genres=${genreId}`,{}, {
      headers: {
        Authorization: `Bearer ${process.env.NEXT_API_TOKEN}`,
      }
    })
...
export async function getPersonsId(person_id) {
  const response = await fetch(
    `https://api.themoviedb.org/3/person/${person_id}?api_key=${process.env.NEXT_API_KEY}`,
