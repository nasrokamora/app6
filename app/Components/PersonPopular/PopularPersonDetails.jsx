...
import AllAutoCarousel from "../AutoCarousel/AllAutoCarousel"
    export default function PopularPersonDetails({ dataPersonPopular }) {
    
        return (
            <div className=" p-4 rounded-lg  flex justify-center items-center w-full h-auto mt-7 md:mt-12 bg-gradient-to-l from-[#ff40c6] via-[#9c40ff] to-[#ff4040] bg-[length:200%_auto] animate-gradient">
                <AllAutoCarousel className="">
                        ...
                        {dataPersonPopular && dataPersonPopular.length > 0 ? (
                            dataPersonPopular.map((person) => (
                                <CarouselItem key={person.id} className="md:basis-1/2 basis-1/6 lg:basis-1/5 2xl:basis-1/6">
                                    <Card className=" p-1 flex flex-col justify-center w-full font-bold border-none md:mt-6 bg-black/30 backdrop-blur">
